...
GeneralSampler::GeneralSampler() 
  ...
  justAfterIntegrate(false), theWriteGridsOnFinish(false) {
    ...
    // Find the single .in file in the current directory
    DIR* dir;
    struct dirent* entry;
    ...
    if ((dir = opendir(".")) != NULL) {
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (filename.size() > 3 && 
                filename.compare(filename.size() - 3, 3, ".in") == 0) {
                in_filename = filename;
                break;  // Take the first .in file found
            }
        }
        closedir(dir);
    }
    ...
    if (in_filename.empty()) {
        std::cout << "No .in file found in the current directory." << std::endl;
          }
    
    // Regular expression pattern to extract process name
    std::regex pattern("do Factory:Process (.+)");
...
void GeneralSampler::rejectLast() {
  cout << "GeneralSampler::rejectLast() : rejecting last point\n";
  if ( !lastSampler() )
    return;
  double w = 0.0;
  if ( weighted() )
...
generator()->log() <<"This corresponds to a cross section difference between:\n"
  ...
  double runXSecErr =
    ...
    << "  \"CrossSectionRun\": " << runXSec << ",\n"
    << "  \"CrossSectionRunError\": " << sqrt(runXSecErr) << ",\n"
    << "  \"PointsAttempted\": " << theAttempts << ",\n"
    << "  \"PointsAccepted\": " << theAccepts << ",\n"
